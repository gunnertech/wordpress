...
if( $domain_mapping_id ) {
  ...
  // add_filter('upload_dir', function($upload) use ($blog_id) {    
  //   $upload['subdir'] = "/" . $blog_id;
...
add_filter("pre_site_option_ms_files_rewriting", function($value) {
  return null;
});


add_filter("wp_get_attachment_url", function($url) {
  if(isset($url) && strpos($url,"/wp-content/uploads/") !== FALSE) {
    $url = preg_replace('/\/wp-content\/uploads\/sites\/\d+\/(2010|2011|2012)/i','/files/$1',$url);
  }
  return $url;
});
