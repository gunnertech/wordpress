...
function hbgs_get_additional_meta_boxes() {
  return array(
    ...
    array(
      ...
      "callback" => function() { 
        global $post;
        $hbgs_meta = get_post_meta($post->ID,'_hbgs_meta',TRUE);
        $hbgs_meta = is_string($hbgs_meta) ? array() : $hbgs_meta;
        echo '<div class="">
          <div>
            ...
            <input class="upload_image_button" type="button" value="Upload Image" />'.$hbgs_meta["header_url"].'dddd</p>
          </div>
          <div>
            ...
            <select name="_hbgs_meta[post_template]">
              <option value=""></option>
                ...
                <?php while (false !== ($file = readdir($handle))): ?>
                  <?php if ($file != "." && $file != ".."): ?>
                    <option value="<?php echo $file ?>" <?php selected($hbgs_meta['post_template'], $file) ?>><?php echo $file ?></option>
                  <?php endif; ?>
                <?php endwhile; ?>
...
function hbgs_meta_save($post_id) {
  ...
  if (!array_key_exists('hbgs_meta_noncename',$_POST)) return $post_id;
	...
	if ($current_data) {
		...
		if(!add_post_meta($post_id,'_hbgs_meta',$new_data,TRUE)) {
		  delete_post_meta($post_id,'_hbgs_meta');
