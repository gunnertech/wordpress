...
class Hbgs_Widgets_Thumbnail_Carousel extends Hbgs_Widget {
  ...
  function __construct() {
  	$widget_ops = array( 'classname' => 'thumbnail-carousel-widget', 'description' => 'A widget that lets you build your own query and display items based on that.' );
  	$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'thumbnail-carousel-widget' );
  	parent::__construct( 'thumbnail-carousel-widget', 'Thumbnail Carousel', $widget_ops, $control_ops );
  }
  ...
  function parent_update( $new_instance, $old_instance ) {
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['the_template'] = 'thumbnail_carousel.php';
		if(isset($instance['template_parameters'])) {
      if(strpos($instance['template_parameters'],'tag=ul') === FALSE) {
	...
	function render( $args, $instance ) {
	  $instance['wrapper_open'] = '<div class="t-carousel">';
	  $instance['wrapper_close'] = '</div>';
  ...
  function print_scripts($id,$instance,$scripts=null) {
    $instance['scripts'] .= '
    $("#widget-id .t-carousel img").live("click",function() {
      var a;
      if(a = $(this).parents("li").find("a").trigger("click").get(0)) {
        location.href = a.href;
      }
    });
    $(function(){
      $("#widget-id .t-carousel img").each(function() {
        var a;
        if(a = $(this).parents("li").find("a").trigger("click").get(0)) {
      ...
      $("#widget-id .t-carousel").infiniteCarousel({
        displayThumbnailNumbers: false,
        thumbnailWidth: "130px",
        thumbnailHeight: "85px",
        textholderHeight: .25,
        imagePath: "/wp-content/themes/html5_boilerplated_grid_system/images/infinitecarousel/"
      });
      
      $("#widget-id .textholder").css("cursor","pointer").live("click",function(){
  ...
  function print_default_styles() {
    wp_enqueue_style( 'thumbnail-carousel', get_bloginfo('template_url').'/css/infinitecarousel2.css');
  }
  
  function print_default_scripts() { 
    wp_enqueue_script( 'thumbnail-carousel', get_bloginfo('template_url').'/js/libs/jquery.infinitecarousel2.min.js', array('jquery'));
  }
  ...
  function parent_form( $instance ) { 
	...
	function form( $instance ) { 
	  $instance['template_parameters'] = isset($instance['template_parameters']) ? $instance['template_parameters'] : '';
	  if(strpos($instance['template_parameters'],'tag=ul') === FALSE) {
	    $instance['template_parameters'] .= '&tag=ul';
	  }
	  $instance['the_template'] = 'thumbnail_carousel.php';
	  ?>
	  <div class="thumbnail_carousel_form">
