...
function optionsframework_init() {
	...
	if ( empty($_POST) || !wp_verify_nonce($_POST['mtheme_options_nonce'], basename(__FILE__))) {
		...
		if (isset( $_POST['update'] )) {
			...
			// IMPORT STORED THEME OPTIONS
			$importpack=array();
			$importpack = $_POST[$option_name]['importpack'];

			if ( $importpack<>"" && is_serialized($importpack) ) {

				add_settings_error( 'options-framework', 'restore_defaults', __( 'Options Imported!', 'optionsframework' ), 'updated fade' );
				...
				add_settings_error( 'options-framework', 'restore_defaults', __( 'Invalid Import', 'optionsframework' ), 'error fade' );
			} else {
				add_settings_error( 'options-framework', 'restore_defaults', __( 'Options Saved!', 'optionsframework' ), 'updated fade' );
			}
