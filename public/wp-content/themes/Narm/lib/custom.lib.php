...
**/
function narm_comment($comment, $args, $depth) 
{
...
function narm_substr($str, $length, $minword = 3)
{
    $sub = '';
    $len = 0;
...
**/
function narm_posts($sort = 'recent', $echo = TRUE) 
{
	...
	if($sort == 'recent')
	{
		$posts = get_posts('numberposts=3&order=DESC&orderby=date&category='.$nm_blog_cat);
		$title = 'Recent Posts';
	}
	else
	{
		...
		$query.= "LEFT OUTER JOIN wp_terms t ON t.term_id = r.term_taxonomy_id WHERE t.term_id = ".$nm_blog_cat." AND p.post_type = 'post' ";
	...
	if(!empty($posts))
	{
		...
		$return_html.= '<ul class="posts blog">';
			...
			foreach($posts as $post)
			{
				...
				$return_html.= '<li><img src="'.get_bloginfo( 'stylesheet_directory' ).'/timthumb.php?src='.$image_thumb.'" style="height:75px; width:75px;" alt="" /> <a href="'.get_permalink($post->ID).'">';
				$return_html.= $post->post_title.'</a><br />';
				$return_html.= narm_substr(strip_shortcodes($post->post_content), 80).'</li>';
...
function get_the_content_with_formatting ($more_link_text = '(more...)', $stripteaser = 0, $more_file = '') {
	...
	$content = _substr($content, 600);
	$content.= '<br/><br/><a class="long_button" href="'.get_permalink().'"><span>Read More</span></a>';
	return $content;
}

function get_the_excerpt_with_formatting ($more_link_text = '(more...)', $stripteaser = 0, $more_file = '') {
